...
@CommandLine.Command(
    ...
    sortOptions = false,
    description =
        "Bulk copy Polaris management entities. Entities will not be removed from the source Polaris instance.")
public class CopyPolarisCommand implements Callable<Integer> {
  ...
  @CommandLine.Command(
      name = "catalogs",
      description =
          "Copy catalogs and optionally catalog subentities like catalog roles, grants, "
  ...
  public Integer migrateCatalogs(
      ...
      @CommandLine.Option(names = {"--migrate-catalog-role-assignments"})
          boolean migrateCatalogRoleAssignments)
      throws Exception {
    doTimedRunWithStatistics(
        m -> {
          try {
            return m.migrateCatalogs(
                migrateCatalogRoles, migrateCatalogRoleAssignments, migrateGrants);
          } catch (Exception e) {
            throw new RuntimeException(e);
          }
        });
  ...
  public Integer migratePrincipals(
      @CommandLine.Option(names = {"--migrate-principal-role-assignments"})
  ...
  @CommandLine.Command(name = "principal-roles", description = "Copy principal roles.")
  ...
  private void doTimedRunWithStatistics(Function<ManagementMigrator, MigrationLog> runMigrator)
      throws Exception {
    ...
    consoleLog.info("Report:\n{}", log.generateReport());

    consoleLog.info("Execution took {}", stopWatch);
  }
}
